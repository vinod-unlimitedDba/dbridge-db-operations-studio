...
export default function Home() {
  ...
  const filteredLogs = useMemo(() => logs.filter((log) => (level === "ALL" || log.level === level) && `${log.source} ${log.message}`.toLowerCase().includes(search.toLowerCase())), [logs, level, search]);
      ...
      <section className="workspace">
        ...
        <div className="content">
          ...
          <section className="log-panel">
            <div className="log-head"><div><span className="live-dot"/><h2>Live logs</h2><span className="live-badge">LIVE</span><small>{filteredLogs.length} events</small></div><div className="log-tools"><button onClick={() => setPaused(!paused)}>{paused ? "▶ Resume" : "Ⅱ Pause"}</button><button onClick={copyLogs}>{copied ? "✓ Copied" : "▣ Copy"}</button><button onClick={() => setLogs([])}>Clear</button><button>⌃</button></div></div>
