...
import ExecutionPlanInvestigator from "./components/ExecutionPlanInvestigator";
import KeepPassVault, { type KeepPassEntry } from "./components/KeepPassVault";

type View = "overview" | "studio" | "infrastructure" | "performance" | "runbooks";
type StudioTool = "sql" | "vault" | "mongodb" | "diagnostics" | "observability" | "intelligence" | "devops" | "investigation";
...
type SqlWorksheet = { id:string; name:string; engine:string; content:string; dirty:boolean; cursor:number; parentId:string|null };
...
type StudioAdvancedProfile = { outcome:string; evidence:string; guardrail:string; featured:string[] };
...
const studioToolOrder:StudioTool[]=["sql","vault","mongodb","diagnostics","observability","intelligence","devops","investigation"];
const studioToolMeta:Record<StudioTool,StudioToolMeta>={
  vault:{mark:"KP",label:"Keep Pass",summary:"Encrypted credentials + URLs",purpose:"Store database, SSH, DevOps, web, and generic credentials in a KeePass-style encrypted local vault.",context:"Master password and Web Crypto",steps:["Create or unlock the local encrypted vault.","Add a target, URL, username, password, tags, and notes.","Launch it into SQL Workspace, MongoDB Studio, DevOps, SSH, or a safe browser tab."],modes:[{id:"entries",label:"Vault entries",detail:"Searchable encrypted credential library"},{id:"generator",label:"Password generator",detail:"Strong local password generation"},{id:"backup",label:"Encrypted backup",detail:"Portable encrypted import and export"}]},
...
const studioAdvancedProfiles:Record<StudioTool,StudioAdvancedProfile>={
  vault:{outcome:"Keep every approved target and secret in one encrypted local vault.",evidence:"AES-GCM ciphertext, auto-lock, encrypted backups and launch history.",guardrail:"The master password is never stored or sent to the local agent.",featured:["entries","generator","backup"]},
  sql:{outcome:"Move from draft SQL to bounded, explainable execution.",evidence:"SQL Guard, result statistics, history, catalog context.",guardrail:"Read-only default; writes require explicit unlock.",featured:["editor","history","scripts"]},
  mongodb:{outcome:"Explore, explain, and migrate document workloads safely.",evidence:"Schema samples, index use, scan ratio, sync verification.",guardrail:"Read-only explorers; cutover remains separately confirmed.",featured:["documents","aggregations","explain","performance","sync"]},
  diagnostics:{outcome:"Turn a symptom into ranked, engine-native evidence.",evidence:"Incident correlation, X-Ray, runtime traces, plan deltas.",guardrail:"Unavailable evidence is never reported as healthy.",featured:["incident","xray","plans","runtime","deepdive"]},
  observability:{outcome:"Correlate logs, traces, telemetry, and migration proof.",evidence:"Redlines, resource hints, remote tails, TKPROF output.",guardrail:"Paste mode stays local; remote collection is bounded.",featured:["compare","native","trace","telemetry","tkprof"]},
  intelligence:{outcome:"Apply the right performance pack for each database.",evidence:"Bottlenecks, statements, plan regression, HA and lag.",guardrail:"Recommendations remain evidence-backed and reversible.",featured:["bottleneck","xray","plans","resilience","goldengate"]},
  devops:{outcome:"Inspect platforms and remote hosts from one console.",evidence:"Cluster, container, delivery, config, SSH and GitOps state.",guardrail:"Read first; preview and confirm controlled changes.",featured:["kubernetes","docker","kargo","ssh","changes"]},
  investigation:{outcome:"Preserve incident evidence and make response repeatable.",evidence:"Recorder, timeline, runbooks, rules and local artifacts.",guardrail:"Credentials and sensitive output are excluded from libraries.",featured:["recorder","timeline","runbooks","rules","library"]},
};
...
export default function Home(){
  ...
  const [sqlTabs,setSqlTabs]=useState<SqlWorksheet[]>([{id:"tab-studio-main",name:"query_01.sql",engine:"postgres",content:sqlSamples.postgres,dirty:false,cursor:0,parentId:null}]);
  ...
  const [studioPins,setStudioPins]=useState<string[]>([]);
  const [studioCockpitOpen,setStudioCockpitOpen]=useState(true);
...
useEffect(()=>{if(!agentToken)return;agentCall("/api/tools/status").then(data=>setToolInventory(data.tools||{})).catch(()=>setToolInventory({}))},[agentToken]);
  ...
  useEffect(()=>{if(!agentToken)return;agentCall("/api/editor/session").then(data=>{const restored:SqlWorksheet[]=(data.session?.tabs||[]).map((tab:any,index:number)=>({id:String(tab.id||`tab-restored-${index}`),name:String(tab.name||`query_${index+1}.sql`),engine:String(tab.engine||form.engine),content:String(tab.content||""),dirty:false,cursor:Number(tab.cursor||0),parentId:typeof tab.parentId==="string"?tab.parentId:null}));if(!restored.length)return;const ids=new Set(restored.map(tab=>tab.id));const normalized=restored.map(tab=>tab.parentId&&!ids.has(tab.parentId)?{...tab,parentId:null}:tab);const active=normalized.find(tab=>tab.id===data.session.activeId)||normalized[0];setSqlTabs(normalized);setActiveSqlTabId(active.id);setSql(active.content)}).catch(()=>{})},[agentToken]);
  ...
  useEffect(()=>{try{const pins=JSON.parse(localStorage.getItem("dbops.studio.pins.v1")||"[]");if(Array.isArray(pins))setStudioPins(pins.filter(item=>typeof item==="string").slice(0,12))}catch{/* pinned tools are optional */}const open=localStorage.getItem("dbops.studio.cockpit-open.v1");if(open!==null)setStudioCockpitOpen(open!=="false")},[]);
  ...
  const addSqlTab=()=>{if(sqlTabs.length>=20)return notify("The workspace is limited to 20 SQL sheets");const id=`tab-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,8)}`;const next:SqlWorksheet={id,name:`query_${String(sqlTabs.filter(tab=>!tab.parentId).length+1).padStart(2,"0")}.sql`,engine:form.engine,content:sqlSamples[form.engine]||"SELECT *\nFROM table_name\nLIMIT 50;",dirty:true,cursor:0,parentId:null};setSqlTabs(items=>[...items,next]);setActiveSqlTabId(id);setSql(next.content);setResult(null);setResultFilter("")};
  const addSqlSubSheet=()=>{if(sqlTabs.length>=20)return notify("The workspace is limited to 20 SQL sheets");const active=sqlTabs.find(item=>item.id===activeSqlTabId);if(!active)return;const parent=active.parentId?sqlTabs.find(item=>item.id===active.parentId)||active:active;const children=sqlTabs.filter(item=>item.parentId===parent.id);if(children.length>=10)return notify("A worksheet can contain up to 10 sub-sheets");const id=`tab-sub-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,8)}`;const base=parent.name.replace(/\.sql$/i,"");const next:SqlWorksheet={id,name:`${base}.variant_${String(children.length+1).padStart(2,"0")}.sql`,engine:active.engine,content:active.id===activeSqlTabId?sql:active.content,dirty:true,cursor:0,parentId:parent.id};const parentIndex=sqlTabs.findIndex(item=>item.id===parent.id);let insertAt=parentIndex+1;while(insertAt<sqlTabs.length&&sqlTabs[insertAt].parentId===parent.id)insertAt+=1;setSqlTabs(items=>[...items.slice(0,insertAt),next,...items.slice(insertAt)]);setActiveSqlTabId(id);setSql(next.content);setResult(null);setResultFilter("");notify(`Sub-sheet created from ${parent.name}`)};
  const closeSqlTab=(id:string)=>{if(sqlTabs.length===1)return notify("Keep at least one SQL worksheet open");const closing=sqlTabs.find(item=>item.id===id);if(!closing)return;if(!closing.parentId&&sqlTabs.some(item=>item.parentId===id))return notify("Close or move its sub-sheets first");const index=sqlTabs.findIndex(item=>item.id===id);const next=sqlTabs.filter(item=>item.id!==id);setSqlTabs(next);if(activeSqlTabId===id){const fallback=(closing.parentId&&next.find(item=>item.id===closing.parentId))||next[Math.max(0,index-1)]||next[0];setActiveSqlTabId(fallback.id);setSql(fallback.content)}};
  ...
  useEffect(()=>{const handler=(event:KeyboardEvent)=>{if((event.ctrlKey||event.metaKey)&&event.altKey&&event.key.toLowerCase()==="n"){event.preventDefault();event.shiftKey?addSqlSubSheet():addSqlTab()}};window.addEventListener("keydown",handler);return()=>window.removeEventListener("keydown",handler)},[sqlTabs,activeSqlTabId,sql,form.engine]);
...
const copyResult=async()=>{if(!result)return;const content=result.columns.length?[result.columns.join("\t"),...result.rows.map(row=>row.join("\t"))].join("\n"):result.raw||result.stderr;try{await navigator.clipboard.writeText(content);notify("Result copied")}catch{notify("Clipboard access was blocked by the browser")}};
  ...
  const passDevopsUrl=(urlValue=devopsUrl,usernameValue=devopsUsername,passwordValue=devopsPassword)=>{
    ...
    setStudioTool("devops");
    if(protocol!=="ssh:"&&(usernameValue.trim()||passwordValue))return notify("Username and password fields are for SSH only; GitHub, Kubernetes, Kargo, and Docker use approved local CLI contexts");
    ...
    if(protocol==="ssh:"){
      const username=(usernameValue.trim()||decodeURIComponent(parsed.username)).trim();
      ...
      setSshConnectionRequest({url:parsed.toString(),requestId:Date.now(),username,password:passwordValue,remember:Boolean(passwordValue)||keepPass,autoConnect:true});
  ...
  const launchVaultDatabase=async(entry:KeepPassEntry)=>{
    if(agentState!=="online")return notify("Start the local database agent before connecting from Keep Pass");
    const targetEnvironment=(environments.includes(entry.environment as Environment)?entry.environment:environment) as Environment;
    const defaults=adapterDefaults[entry.engine];if(!defaults)return notify("This vault entry uses an unsupported database engine");
    const credentialId=`vault-${entry.id}`.slice(0,128);
    try{
      if(entry.password)await agentCall("/api/credentials/session",{method:"POST",body:JSON.stringify({scope:"database",id:credentialId,username:entry.username,password:entry.password})});
      const value:DbForm={environment:targetEnvironment,engine:entry.engine,authMode:entry.password?"password":"context",tlsMode:entry.tlsMode||"require",host:entry.host,port:entry.port||defaults.port,database:entry.database||defaults.database,username:entry.username,password:""};
      setEnvironment(targetEnvironment);setForm(value);setProfileId("");setActiveCredentialId(entry.password?credentialId:"");setConnectionState("idle");setConnectionFingerprint("");setCatalog([]);
      const destination:StudioTool=entry.engine==="mongodb"?"mongodb":"sql";setStudioTool(destination);
      const connected=await connect(true,value,targetEnvironment,entry.password?credentialId:"");if(!connected){openConnectionPanel();return}
      setConnectionPanelOpen(false);if(destination==="sql")requestAnimationFrame(()=>editorRef.current?.focus());notify(`${defaults.name} connected from Keep Pass`);
    }catch(error){notify(error instanceof Error?error.message:"Keep Pass database launch failed")}
  };
  const launchVaultSsh=(entry:KeepPassEntry)=>{const host=entry.host.trim();if(!host&&!entry.url.trim())return notify("This SSH vault entry has no hostname or URL");const url=entry.url.trim()||`ssh://${host}:${entry.port||"22"}`;setStudioTool("devops");setDevopsMode("ssh");setDevopsData(null);setSshConnectionRequest({url,requestId:Date.now(),username:entry.username,password:entry.password,remember:true,autoConnect:true});notify("Keep Pass sent the SSH target for host-key inspection and connection")};
  const launchVaultDevops=(entry:KeepPassEntry)=>passDevopsUrl(entry.url,entry.username,entry.password);
  const clearVaultRuntimeCredentials=()=>{setForm(current=>({...current,password:""}));setDevopsPassword("");setMirrorPassword("");if(agentToken)void agentCall("/api/credentials/session/clear",{method:"POST",body:"{}"}).catch(()=>{})};
  ...
  const studioStatus=studioTool==="vault"
    ?"AES-256 local vault ready"
    :studioTool==="observability"
  ...
  const activeStudioModeId:Record<StudioTool,string>={
    sql:sqlInspectorOpen&&(sqlInspector==="history"||sqlInspector==="scripts")?sqlInspector:"editor",vault:"entries",
    mongodb:mongoMode,diagnostics:diagnosticMode,observability:observeMode,intelligence:intelligenceMode,devops:devopsMode,investigation:investigationMode,
  };
  const studioAdvancedProfile=studioAdvancedProfiles[studioTool];
  const activeStudioMode=activeStudioMeta.modes.find(mode=>mode.id===activeStudioModeId[studioTool])||activeStudioMeta.modes[0];
  const activeStudioModeIndex=Math.max(0,activeStudioMeta.modes.findIndex(mode=>mode.id===activeStudioMode.id));
  const nextStudioMode=activeStudioMeta.modes[(activeStudioModeIndex+1)%activeStudioMeta.modes.length];
  const studioCockpitScore=studioStatusTone==="ready"?100:70;
  const featuredStudioModes=studioAdvancedProfile.featured.map(id=>activeStudioMeta.modes.find(mode=>mode.id===id)).filter((mode):mode is StudioModeEntry=>Boolean(mode));
  const pinnedStudioItems=studioPins.map(key=>studioCommandItems.find(item=>item.key===key)).filter((item):item is StudioCommandItem=>Boolean(item?.mode));
  const toggleStudioPin=(key:string)=>setStudioPins(current=>{const next=current.includes(key)?current.filter(item=>item!==key):[key,...current].slice(0,12);localStorage.setItem("dbops.studio.pins.v1",JSON.stringify(next));notify(current.includes(key)?"Advanced tool unpinned":"Advanced tool pinned");return next});
  const toggleStudioCockpit=()=>setStudioCockpitOpen(current=>{const next=!current;localStorage.setItem("dbops.studio.cockpit-open.v1",String(next));return next});
  const openNextStudioMode=()=>openStudioLocation(studioTool,nextStudioMode.id);
  ...
  return <main className={`ops-shell ${workspaceWide?"workspace-wide":""}`}>
    ...
    <section className="ops-app">
      ...
      <div className="ops-scroll">
        {view==="studio"&&<div className="ops-view">
          ...
          <section className={`studio-tab-cockpit ${studioCockpitOpen?"open":"collapsed"}`} aria-label={activeStudioMeta.label+" advanced cockpit"}>
            <header><div className="cockpit-identity"><span>{activeStudioMeta.mark}</span><p><small>ADVANCED TAB / {environment.toUpperCase()}</small><b>{activeStudioMode.label}</b><em>{activeStudioMode.detail}</em></p></div><div className="cockpit-score" style={{"--cockpit-score":studioCockpitScore+"%"} as any}><b>{studioCockpitScore}</b><small>READY</small></div><aside><button className="cockpit-continue" onClick={openNextStudioMode}>Continue: {nextStudioMode.label}<span>-&gt;</span></button><button onClick={toggleStudioCockpit}>{studioCockpitOpen?"Collapse":"Expand"}</button></aside></header>
            {studioCockpitOpen&&<div className="cockpit-body">
              <section className="cockpit-brief"><div><small>OPERATOR OUTCOME</small><b>{studioAdvancedProfile.outcome}</b><span>{studioAdvancedProfile.evidence}</span></div><em>{studioAdvancedProfile.guardrail}</em></section>
              <div className="cockpit-gates">{[["ENV",environment,"Environment scope applied",true],["ACCESS",studioStatus,activeStudioMeta.context,studioStatusTone==="ready"],["SAFETY",allowWrites&&studioTool==="sql"?"Writes explicitly unlocked":"Read-only default","Sensitive actions stay gated",true],["FLOW",(activeStudioModeIndex+1)+"/"+activeStudioMeta.modes.length,activeStudioMode.label,true]].map(([mark,value,detail,ready])=><article key={String(mark)} className={ready?"ready":"attention"}><span>{String(mark)}</span><p><b>{String(value)}</b><small>{String(detail)}</small></p><em>{ready?"READY":"ACTION"}</em></article>)}</div>
              <section className="cockpit-workflow"><header><div><small>FASTEST SAFE PATH</small><b>{activeStudioMeta.label} operating sequence</b></div><span>Phase {Math.min(activeStudioModeIndex+1,activeStudioMeta.steps.length)} of {activeStudioMeta.steps.length}</span></header><div>{activeStudioMeta.steps.map((step,index)=><article key={step} className={index===Math.min(activeStudioModeIndex,activeStudioMeta.steps.length-1)?"active":index<activeStudioModeIndex?"done":""}><span>{String(index+1).padStart(2,"0")}</span><p>{step}</p><i/></article>)}</div></section>
              <section className="cockpit-launcher"><header><div><small>POWER TOOLS</small><b>Best advanced workflows for this tab</b></div><span>{studioPins.length} pinned across Studio</span></header><div>{featuredStudioModes.map(mode=>{const key=studioTool+":"+mode.id,pinned=studioPins.includes(key);return <article key={mode.id} className={activeStudioMode.id===mode.id?"active":""}><button onClick={()=>openStudioLocation(studioTool,mode.id)}><span>{activeStudioMeta.mark}</span><p><b>{mode.label}</b><small>{mode.detail}</small></p><em>OPEN</em></button><button className={pinned?"pin active":"pin"} onClick={()=>toggleStudioPin(key)}>{pinned?"UNPIN":"PIN"}</button></article>})}</div></section>
              {pinnedStudioItems.length>0&&<nav className="cockpit-pins" aria-label="Pinned advanced tools"><span>PINNED</span>{pinnedStudioItems.map(item=><button key={item.key} onClick={()=>openStudioLocation(item.tool,item.mode)}><i>{item.mark}</i><b>{item.label}</b><small>{studioToolMeta[item.tool].label}</small></button>)}</nav>}
            </div>}
          </section>
          <section className={`studio-grid ${studioTool==="devops"||studioTool==="vault"?"without-connection":connectionPanelOpen?"connection-open":"connection-collapsed"}`}>
            <div hidden={studioTool!=="vault"} className="keep-pass-mount"><KeepPassVault environment={environment} agentOnline={agentState==="online"} agentCall={agentCall} notify={notify} onLaunchDatabase={launchVaultDatabase} onLaunchSsh={launchVaultSsh} onLaunchDevops={launchVaultDevops} onLock={clearVaultRuntimeCredentials}/></div>
            {studioTool!=="devops"&&studioTool!=="vault"&&(connectionPanelOpen?<aside className="db-connector panel-dark" onMouseEnter={cancelConnectionPanelHide} onMouseLeave={scheduleConnectionPanelHide} onFocusCapture={cancelConnectionPanelHide} onBlurCapture={event=>{if(!event.currentTarget.contains(event.relatedTarget as Node))scheduleConnectionPanelHide()}}>
             ...
             {studioTool==="sql"&&<section className="sql-ide panel-dark">
              <header className="ide-tabs"><div className="worksheet-tabs">{sqlTabs.map(tab=>{const parent=tab.parentId?sqlTabs.find(item=>item.id===tab.parentId):null;const childCount=sqlTabs.filter(item=>item.parentId===tab.id).length;return <div key={tab.id} className={`worksheet-tab ${activeSqlTabId===tab.id?"active":""} ${tab.parentId?"sub-sheet":childCount?"parent-sheet":""}`}><button className="worksheet-select" onClick={()=>switchSqlTab(tab.id)} onDoubleClick={()=>renameSqlTab(tab.id)} title={parent?`Sub-sheet of ${parent.name} · double-click to rename`:"Double-click to rename"}><i/>{parent&&<small>SUB</small>}<span>{tab.name}</span>{childCount>0&&<small className="sub-count">{childCount}</small>}{tab.dirty&&<em>●</em>}</button><button className="worksheet-close" onClick={()=>closeSqlTab(tab.id)} aria-label={`Close ${tab.name}`}>×</button></div>})}<button className="worksheet-add" onClick={addSqlTab} title="New worksheet (Ctrl+Alt+N)">+</button><button className="worksheet-add sub-sheet-add" onClick={addSqlSubSheet} title="New sub-sheet from active worksheet (Ctrl+Alt+Shift+N)">+ SUB</button></div><aside><span className={connectionState}><i/>{connectionState==="connected"?`${environment} · ${adapterDefaults[form.engine].name}`:"Not connected"}</span><b>{allowWrites?"Writes unlocked":"Read only"}</b><button className={sqlInspectorOpen?"inspector-toggle active":"inspector-toggle"} onClick={()=>setSqlInspectorOpen(value=>!value)}>Inspector</button></aside></header>
            ...
            {studioTool==="diagnostics"&&<section className="studio-module panel-dark advanced-diagnostics">
              ...
              {diagnosticMode==="plans"&&<div className="module-controls diagnostic-controls epi-live-capture"><label>OPTIONAL LIVE CAPTURE<input value={baselineName} onChange={event=>setBaselineName(event.target.value)} placeholder="Approved plan baseline"/><small>Use the paired agent for a retained engine plan, or upload/paste plans below without a database connection.</small></label><div><label>ENGINE EVIDENCE<strong>Capture, history and regression context</strong><small>Oracle cursor cache, PostgreSQL pg_stat_statements, MongoDB profiler, MySQL/MariaDB Performance Schema, and SQL Server Query Store.</small></label></div><div className="version-actions"><button className="primary" onClick={()=>runIntelligence("capture")} disabled={intelligenceBusy}>Capture current plan</button><button className="secondary" onClick={()=>runIntelligence("history")} disabled={intelligenceBusy}>Plan history</button><button className="secondary" onClick={savePlanBaseline} disabled={!intelligenceData?.planText}>Save captured baseline</button></div></div>}
              ...
              <section className="module-body diagnostic-body">
                ...
                {diagnosticMode==="deepdive"&&(!intelligenceData?<div className="module-empty"><span>360</span><b>{diagnosticLabels[diagnosticMode]} is ready</b><small>Run the complete engine analyzer to build a pressure map, dominant mode and cause/impact/verification chain.</small></div>:<PerformanceEvidence data={intelligenceData} engine={form.engine}/>) }
                {diagnosticMode==="plans"&&<><ExecutionPlanInvestigator engine={form.engine} capturedPlan={intelligenceData?.planText||""} onNotify={notify}/>{intelligenceData&&!intelligenceData.planText&&<PerformanceEvidence data={intelligenceData} engine={form.engine}/>}</>}
    ...
    {commandOpen&&<div className="command-backdrop" onMouseDown={event=>event.target===event.currentTarget&&setCommandOpen(false)}><section className="command-palette advanced" role="dialog" aria-label="Studio Command Center">
      ...
      <footer><span><kbd>Alt 1-8</kbd> switch Studio tabs</span><span><kbd>Ctrl Shift F</kbd> wide canvas</span><span><kbd>Esc</kbd> close</span></footer>
    </section></div>}
